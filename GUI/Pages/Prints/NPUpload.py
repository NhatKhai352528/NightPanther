...
class NPUpload(NPPages):
    ...
    def __init__(self, master: Tk, commands: list[Any, Any] = None, serverLink: str = None, serverKey: str = None, serverQRFile: str = None, fileName: str = None):
        ...
        self._fileName = fileName if fileName != None else self._currentLanguage["upload"]["data"]["text4"]
        
        # Initialize items for data frame
        self._data.initText(mode = "title", text = self._currentLanguage["upload"]["data"]["title"], wrap = True)
        
        self._data.initText(mode = "heading", text = self._currentLanguage["upload"]["data"]["text0"], wrap = False)
        self._serverLinkIndex = self._data.initText(mode = "content", text = self._currentLanguage["upload"]["data"]["text1"] + self._serverLink, wrap = False)
        self._serverKeyIndex = self._data.initText(mode = "content", text = self._currentLanguage["upload"]["data"]["text2"] + self._serverKey, wrap = False)
        
        self._data.initText(mode = "Heading", text = self._currentLanguage["upload"]["data"]["text3"], wrap = False)
        self._fileNameIndex = self._data.initText(mode = "Content", text = self._fileName, wrap = True)
        
        # Initialize items for control frame
        # self._control.initButton(position = "left", command = self._commands[0], state = "normal", text = self._currentLanguage["upload"]["control"]["left"])
        self._control.initButton(position = "right", command = self._commands[1], state = "normal", text = self._currentLanguage["upload"]["control"]["right"])
    ...
    def npset(self, attribute: str, value: Any = None):
        if attribute == "serverLink":
            ...
            self._data.updateText(index = self._serverLinkIndex, text = self._currentLanguage["upload"]["data"]["text1"] + self._serverLink)
        elif attribute == "serverKey":
            self._serverKey = value
            self._data.updateText(index = self._serverKeyIndex, text = self._currentLanguage["upload"]["data"]["text2"] + self._serverKey)

...
class NPOrder(NPPages):
    ...
    def __init__(self, master: Tk, commands: list[Any, Any] = None, fileName: str = None, filePrice: int = None):
        ...
        self._data.initText(mode = "title", text = currentLanguage["order"]["data"]["title"], wrap = True)
        
        self._data.initText(mode = "heading", text = currentLanguage["order"]["data"]["text0"], wrap = False)
        self._fileNameIndex = self._data.initText(mode = "content", text = self._fileName, wrap = False)
        
        self._data.initText(mode = "heading", text = currentLanguage["order"]["data"]["text1"], wrap = False)
        self._filePriceIndex = self._data.initText(mode = "content", text = formatPrice(self._filePrice), wrap = False)
        
        self._data.initText(mode = "Heading", text = currentLanguage["order"]["data"]["text2"], wrap = False)
        self._userPriceIndex = self._data.initText(mode = "Content", text = formatPrice(self._userPrice), wrap = True)
        
        # Initialize items for control frame
        self._control.initButton(position = "left", command = self._commands[0], state = "normal", text = currentLanguage["order"]["control"]["left"])
